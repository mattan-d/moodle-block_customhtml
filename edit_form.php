...
class block_customhtml_edit_form extends block_edit_form {
    protected function specific_definition($mform) {
        ...
        $mform->addElement('text', 'config_title', get_string('configtitle', 'block_customhtml'));
        ...
        if (!empty($CFG->block_customhtml_allowcssclasses)) {
            ...
            $mform->addHelpButton('config_classes', 'configclasses', 'block_customhtml');
        }
    }
    ...
    function set_data($defaults) {
        if (!empty($this->block->config) && !empty($this->block->config->text)) {
            ...
            $defaults->config_text['text'] = file_prepare_draft_area($draftid_editor, $this->block->context->id, 'block_customhtml', 'content', 0, array('subdirs'=>true), $currenttext);
